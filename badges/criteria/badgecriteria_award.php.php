...
abstract class badgecriteria_award {
    ...
    /**
     * Supported badge types (e.g. BADGE_TYPE_COURSE, BADGE_TYPE_SITE).
     * @var array
     */
        ...
        if (isset($this->params)) {
            if (count($this->params) > 1) {
                echo $OUTPUT->box(get_string('description', 'badgecriteria_' . $this->criteriatype,
                        core_text::strtoupper($agg[$badge->get_aggregation_method($this->criteriatype)])), array('clearfix'));
            } else {
    ...
    public static function config_form_invalid_criteria(badge $badge, $name) {
        ...
        echo html_writer::tag('span', get_string('invalidcriteria_descr_full', 'badges', $name), array('class' => 'error'));
